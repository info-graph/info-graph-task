...
export const updateMaintenanceRecords = createAsyncThunk(
  'maintenance/update',
  async ({ id, maintenanceData }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`${API_URL}/api/maintenance/${id}`, maintenanceData);
      return response.data;
    } catch (error) {
...
export const deleteMaintenance = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await axios.delete(`${API_URL}/api/maintenance/${id}`);
...
const maintenanceSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateMaintenanceRecords.pending, (state) => {
        state.loading = true;
        state.error = null;
        state.success = false;
      })
      .addCase(updateMaintenanceRecords.fulfilled, (state, action) => {
        state.loading = false;
      ...
      .addCase(updateMaintenanceRecords.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || 'Something went wrong';
      })
